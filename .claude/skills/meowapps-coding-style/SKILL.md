---
description: Review or refactor code against meowapps coding style rules and conventions.
---

## What

Review or refactor code against meowapps coding style rules and conventions.

## Why

The rules encode the project's philosophy: less code, functions own their world, code for maintenance. Inconsistent code erodes these principles and makes the framework harder to maintain for everyone downstream.

## Who

Both framework maintainers and app developers. Same rules apply to all meowapps code.

## When

When reviewing, writing, or refactoring code in this project or in apps built with meowapps.

## Where

Framework code in `scripts/` and `src/`. App developer code in their `src/`.
...
1-2 lines of "what" before function declaration. Inside, only "why" for non-obvious logic.
```js
...
State/setup → return → helper functions. Reader sees *what* renders first, *how* it works after.
```jsx
function Index() {
...
- Ask: which files? All rules or specific ones?
- Read the files
- Check against rules and conventions — only flag violations of the above, not general code quality
- Report violations: file, line, rule number, what's wrong, proposed fix
- Get approval, then apply fixes
